...
以下的 `printf()` 实现来自于 [xv6](https://github.com/mit-pdos/xv6-riscv/blob/riscv/kernel/printf.c)。`printf()` 函数参数个数不定，可以使用 `stdarg.h` 中的 `va_list` 来迭代遍历参数。
