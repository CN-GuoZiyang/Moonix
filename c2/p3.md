...
`__interrupt` 函数通过移动 sp 指针来在栈上留出一个 InterruptContext 的位置，并保存各种寄存器到栈上。如下：

<center>
<img src="https://cn-guoziyang.gitee.io/moonix/assets/img/interruptframe.png" alt="保存中断帧" width=80%>
</center>

随后，将 sp、scause 和 stval 作为参数存入 a0、a1 和 a2，并调用 `handleInterrupt()` 函数。
